...
app.post('/auth/login', function(req, res) {
	...
	database.getUserInfo(data, function(err, userData) {
		...
		if(!err && userData.id == uname && userData.passwd == pwd) {
			...
			var payload = { id: userData.id };
			var token = jwt.sign(payload, app.get('jwtTokenSecret'));
...
app.get('/auth/loginFail', function(req, res) {
...
function getTaskList(projectInfo, callback) {
	...
	database.getTaskList(projectInfo, function(err, taskLists) {
		if(taskLists) {
			...
			taskLists.forEach(function(taskLists) {
				//taskList += taskLists.taskListId + ':';
				
				console.log('---------------');
				console.log('taskListId: ', taskLists.taskListId);
...
//async function getTask(taskListInfo) {
	var database = require('./database-access.js');
	var tasksResults = '';
	
	database.getTask(taskListInfo, function(err, tasks) {
		console.log('===================');
		console.log(taskListInfo.taskListId);
